...
//unsigned char Rcon[22] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
...
unsigned int EncryptBuf(unsigned char *lpInBuf, unsigned int dwDataLen, unsigned char *lpOutBuf, unsigned char *lpKey16Byte) // out buf len = dwDataLen + 16
{
	unsigned char nExpKey[4*Nb*(Nr+1)];
	int i;
	unsigned int dwPackets;
	unsigned char nAlign[16], nRemainder;
	unsigned char *lpIn, *lpOut;
...
unsigned int DecryptBuf(unsigned char *lpInBuf, unsigned int dwDataLen, unsigned char *lpOutBuf, unsigned char *lpKey16Byte) // return decrypt data len
{
	unsigned char nExpKey[4*Nb*(Nr+1)];
	int i;
	unsigned int dwPackets;
	unsigned char nRemainder;
	unsigned char *lpIn, *lpOut;

	if(((dwDataLen % 16) != 0) || (dwDataLen < 16))
	{
		return 0;
	}
...
unsigned int JSHash(unsigned int dwHash, unsigned char *lpData, int len)
{  
	unsigned int hash;
	int i;
...
void HMMakeKey(unsigned char *lpRandom256, unsigned char *lpKey16) // in: Random 256byte, out:key 16byte
{
	unsigned int *lpKey;
